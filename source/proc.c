...
static void build_notify_message(message *m_ptr, int src, struct proc *dst_ptr)
{
...
static int validate_syscall(int call_nr, endpoint_t src_dst_e, struct proc *caller_ptr, int *src_dst_p)
{
    if (call_nr < 0 || call_nr > IPCNO_HIGHEST || call_nr >= 32
        || !(ipc_call_names[call_nr])) {
        ...
        *src_dst_p = (int) src_dst_e;
        ...
        if (call_nr != RECEIVE && !may_send_to(caller_ptr, *src_dst_p)) {
            ...
            printf("sys_call: ipc mask denied %s from %d to %d\n",
                ipc_call_names[call_nr], caller_ptr->p_endpoint, src_dst_e);
#endif
            return ECALLDENIED;
        }
    }
    ...
    if (!(priv(caller_ptr)->s_trap_mask & (1 << call_nr))) {
        ...
        printf("sys_call: %s not allowed, caller %d, src_dst %d\n", 
            ipc_call_names[call_nr], proc_nr(caller_ptr), *src_dst_p);
#endif
        return ETRAPDENIED;
    }
    ...
    if (call_nr != SENDREC && call_nr != RECEIVE && iskerneln(*src_dst_p)) {
        ...
        printf("sys_call: trap %s not allowed, caller %d, src_dst %d\n",
             ipc_call_names[call_nr], proc_nr(caller_ptr), src_dst_e);
#endif
        return ETRAPDENIED;
    }
...
static int do_sync_ipc(struct proc * caller_ptr, int call_nr, endpoint_t src_dst_e, message *m_ptr)
{
    ...
    result = validate_syscall(call_nr, src_dst_e, caller_ptr, &src_dst_p);
    if (result != OK) {
        return result;
    }
...
int mini_send(register struct proc *caller_ptr, endpoint_t dst_e, message *m_ptr, const int flags)
{
