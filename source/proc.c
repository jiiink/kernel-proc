...
int mini_notify(const struct proc *caller_ptr, endpoint_t dst_e) {
    ...
    if (WILLRECEIVE(caller_ptr->p_endpoint, dst_ptr, 0, &m_notify_buff) && !(dst_ptr->p_misc_flags & MF_REPLY_PEND)) {
        assert(!(dst_ptr->p_misc_flags & MF_DELIVERMSG));
...
int try_deliver_senda(struct proc *caller_ptr, asynmsg_t *table, size_t size) {
    ...
    for (i = 0; i < size; i++) {
        ...
        if (r == OK && WILLRECEIVE(caller_ptr->p_endpoint, dst_ptr, (vir_bytes)&table[i].msg, NULL) && (!(flags & AMF_NOREPLY) || !(dst_ptr->p_misc_flags & MF_REPLY_PEND))) {
...
static int try_one(endpoint_t receive_e, struct proc *src_ptr, struct proc *dst_ptr) {
    ...
    for (i = 0; i < size; i++) {
        ...
        if (!CANRECEIVE(receive_e, src_e, dst_ptr, table_v + i * sizeof(asynmsg_t) + offsetof(struct asynmsg, msg), NULL)) {
            continue;
        }
...
static void notify_scheduler(struct proc *p) {
    ...
    if ((err = mini_send(p, p->p_scheduler->p_endpoint, &m_no_quantum, FROM_KERNEL))) {
